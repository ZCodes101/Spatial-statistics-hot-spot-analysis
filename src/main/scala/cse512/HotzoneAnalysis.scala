...
import org.apache.spark.sql.functions.{asc, count}
...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // Query to sel rect & point count from joinResult . order by rect asc
    val hotDf = joinDf
      .groupBy("rectangle")
      .agg(count("*").as("point_count"))
      .orderBy(asc("rectangle"))
      .coalesce(1)  // Merge all part

    // Return hot DataFrame
    hotDf
  }
